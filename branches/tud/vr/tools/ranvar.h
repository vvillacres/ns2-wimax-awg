 ...
 *
 * @(#) $Header: /home/rouil/cvsroot/ns-2.31/tools/ranvar.h,v 1.1.1.1 2008/04/11 18:40:29 rouil Exp $ (Xerox)
 */
...
public:
   ...
   double* locationp() { return &location_; }
   virtual inline double shape() { return shape_; }
   virtual inline double scale() { return scale_; }
   virtual inline double location() { return location_; }
   void setshape(double x) { shape_ = x; }
   void setscale(double x) { scale_ = x; }
   void setlocation(double x) { location_ = x; }
private:
   double shape_;
   double scale_;
   double location_;
...
class GBAR_RandomVariable : public RandomVariable 
{
	public:
		GBAR_RandomVariable();
		GBAR_RandomVariable(double ,double);
		virtual double value();
		virtual double avg();
