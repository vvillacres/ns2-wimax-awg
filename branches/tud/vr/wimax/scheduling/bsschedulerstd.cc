...
static class BSSchedulerStdClass : public TclClass
{
public:
    BSSchedulerStdClass() : TclClass("WimaxScheduler/BS_STD") {}
    TclObject* create(int, const char*const*) {
        return (new BSSchedulerStd());

    }
} class_bsschedulerStd;

/*
 * Create a scheduler
 */
BSSchedulerStd::BSSchedulerStd () : WimaxScheduler ()
{
 ...
 * @return The MAC casted to BSScheduler
 */
Mac802_16BS* BSSchedulerStd::getMac()
{
 ...
 */
int BSSchedulerStd::command(int argc, const char*const* argv)
{
    if (argc == 3) {
 ...
 */
int BSSchedulerStd::getBWopportunity ()
{
 ...
 */
int BSSchedulerStd::getInitRangingopportunity ()
{
...
int BSSchedulerStd::increase_dl_map_ie(int num_of_entries, int totalslots, int num_ie)
{
...
int BSSchedulerStd::overallocation_withdlmap(int num_of_entries, int totalslots, int ownslots)
{
...
int BSSchedulerStd::max_conn_withdlmap(int num_of_entries, int totalslots)
{
...
int BSSchedulerStd::freeslots_withdlmap_given_conn(int num_of_entries, int totalslots, int newconn)
{
...
int BSSchedulerStd::overallocation_withoutdlmap(int num_of_entries, int totalslots, int ownslots)
{
...
void BSSchedulerStd::bubble_sort (int arrayLength, con_data_alloc array[], int sort_field)
{
    int i, j, flag = 1;
    con_data_alloc temp;
...
int BSSchedulerStd::doesvirtual_allocexist(int num_of_entries, int cid)
{
...
int BSSchedulerStd::addslots_withoutdlmap(int num_of_entries, int byte, int slots, int cid)
{
    for (int i = 0; i<num_of_entries-1; i++) {
        if (virtual_alloc[i].cid == cid) {
...
int BSSchedulerStd::check_overallocation(int num_of_entries)
{
 ...
 */
void BSSchedulerStd::schedule ()
{
    debug2("\n\n======================BSSchedulerStd::schedule () Begin ============================\n");
    ...
    for (Connection *n= mac_->getCManager ()->get_in_connection (); n; n=n->next_entry()) {
        if (n->getArqStatus () != NULL && n->getArqStatus ()->isArqEnabled() == 1) {
            if (!(n->getArqStatus ()->arq_feedback_queue_) || (n->getArqStatus ()->arq_feedback_queue_->length() == 0)) {
                ...
                if (peernode->getOutData () != NULL && peernode->getOutData ()->queueLength () != 0 && getMac()->getArqFBinDlData()) {
...
//3. Virtual DCD
    if (getMac()->getSendDCD() || map->getDlSubframe()->getCCC()!= getMac()->getDlCCC()) {
        p = map->getDCD();
        ch = HDR_CMN(p);
...
//4. Virtual UCD
    if (getMac()->getSendUCD() || map->getUlSubframe()->getCCC()!= getMac()->getUlCCC()) {
        p = map->getUCD();
        ch = HDR_CMN(p);
...
/*       // rpi removed this dl burst ----------------------------------------------------------------------------
	 int BSSchedulerStd::addDlBurst (int burstid, Connection *c, int iuc, int dlduration, int maxdlduration)
	 {
	 double txtime; //tx time for some data (in second)
	 int txtime_s;  //number of symbols used to transmit the data
 ...
 * @param the new allocation status
 */
void BSSchedulerStd::addDlBurst (int burstid, int cid, int iuc, int ofdmsymboloffset, int numofdmsymbols, int subchanneloffset, int numsubchannels)
{
    DlBurst *db = (DlBurst*) mac_->getMap()->getDlSubframe()->getPdu ()->addBurst (burstid);
    db->setCid (cid);
    db->setIUC (iuc);
...
int BSSchedulerStd::doesMapExist(int cid, int *cid_list, int num_of_entries)
{
    for (int i = 0; i<num_of_entries; ++i) {
        if (cid_list[i]==cid)
            return i;
...
mac802_16_dl_map_frame * BSSchedulerStd::dl_stage2(Connection *head, int input_subchannel_offset, int total_subchannels, int total_symbols, int symbol_start, int stripping, int total_dl_slots_pusc)
{
...
//1. (1st priority) Allocate management message (basic, primary, secondary)
    ...
    if (mac_->amc_enable_ ==1) {
        for (i=0; i<5; ++i) {
            ...
            while (con!=NULL) {
                if (con->get_category() == CONN_DATA && con->get_serviceflow()->getQoS()->getDataDeliveryServiceType() == dataDeliveryServiceType ) {
...
//2. Calculate #of active connections (ugs, ertps, rtps, nrtps, be)
    ...
    for (int i = 1; i<5; i++) {
    	active_conn_except_ugs =+ conn_per_schetype[i];
    }
    ...
    debug10 ("\tAll UGS <%d>, ertPS <%d>, rtPS <%d>, nrtPS <%d>, BE <%d>\n", con_ugs_all, con_ertvr_all, con_rtvr_all, con_nrtvr_all, con_be_all);
    ...
    for (i=0; i<5; ++i) {
        ...
        while (con!=NULL) {
            if (con->get_category() == CONN_DATA && con->get_serviceflow()->getQoS()->getDataDeliveryServiceType() == dataDeliveryServiceType) {
                ...
                if ( dataDeliveryServiceType==DL_UGS) {
                    ServiceFlowQosSet *sfqos = con->get_serviceflow()->getQoS();
                    allocationsize = int( ceil(( double(sfqos->getMinReservedTrafficRate()) / mac_->getFrameDuration()) / 8 ) );
                    if ( allocationsize > con->queueByteLength() ) {
                    	allocationsize = con->queueByteLength();
                    }
...
//End ARQ aware scheduling
                    ...
                    debug10 ("DL2.UGS, DataSize :%f, period :%d, numslots :%d, CID :%d, DIUC :%d\n", sfqos->getMinReservedTrafficRate(), sfqos->getMaxSustainedTrafficRate(), con_slots, con->get_cid(), con->getPeerNode()->getDIUC());
        ...
        while (con != NULL) {
            ...
            if (con->get_category() == CONN_DATA && con->get_serviceflow()->getQoS()->getDataDeliveryServiceType() == dataDeliveryServiceType) {
                ...
                if ( (arq_enable_f == 1) && (allocationsize > 0) ) {
                    ...
                    debug10 ("\t ARQ enable CID :%d, arq_block :%d, requested_size :%f, arq_block_boundary_size :%f, all_header_included :%f\n", con->get_cid(), getMac()->getArqBlockSize(), ori_grant, withfrag, withfrag + (double)14);
 ...
 * Added by Ritun
 */
diuc_t BSSchedulerStd::getDIUCProfile(Ofdm_mod_rate rate)
{
    Profile *p;
...
/* Added by Ritun
 */
uiuc_t BSSchedulerStd::getUIUCProfile(Ofdm_mod_rate rate)
{
    Profile *p;
...
struct mac802_16_ul_map_frame * BSSchedulerStd::ul_stage2(Connection *head, int total_subchannels, int total_symbols, int symbol_start, int stripping) {
...
//            allocationsize =  RNG_REQ_SIZE+GENERIC_HEADER_SIZE;
    ...
    if (mac_->amc_enable_ ==1) {
        ...
        for (i=0; i<5; ++i) {
            ...
            while (con!=NULL) {
                if (con->get_category() == CONN_DATA && con->get_serviceflow()->getQoS()->getUlGrantSchedulingType() == ulGrantSchedulingType) {
                    /*Xingting would like to change the modulation mode for this SS.*/
        ...
        while (con!=NULL) {
            if (con->get_category() == CONN_DATA && con->get_serviceflow()->getQoS()->getUlGrantSchedulingType() == ulGrantSchedulingType) {
                ...
                if (ulGrantSchedulingType==UL_UGS) {
                    ...
                    double tempLastServiceTime = con->getLastServiceTime();
                    tempLastServiceTime += mac_->getFrameDuration() * 1e3;
                    if ( (tempLastServiceTime / sfqos->getGrantInterval()) >= 1) {
                    	allocationsize = int( ceil( double(sfqos->getMinReservedTrafficRate()) / ( sfqos->getGrantInterval() * 8 )));
                    	if ( allocationsize > con->queueByteLength() ) {
                    		allocationsize = con->queueByteLength();
                    	}

                    	tempLastServiceTime = sfqos->getGrantInterval() * ( tempLastServiceTime - floor(tempLastServiceTime / sfqos->getGrantInterval() ) * sfqos->getGrantInterval());


                    }
                    con->setLastServiceTime( tempLastServiceTime);
                    ...
                    debug10 ("UL.Check1.3.UGS, DataSize :%f, period :%d, PRE-GRANT-SLOTS :%d, Peer-CID :%d, returnCID :%d, DIUC :%d\n", sfqos->getMinReservedTrafficRate(), sfqos->getGrantInterval(), grant_slots, con->getPeerNode()->getBasic(OUT_CONNECTION)->get_cid(), return_cid_tmp, con->getPeerNode()->getDIUC());
                ...
                if (ulGrantSchedulingType == UL_ertPS) {
                    ...
                    */
                    ...
                    if (req_slots_tmp1>0) {
                        allocationsize = ceil((double)sfqos->get()/(double)sfqos->getPeriod());
                ...
                if (ulGrantSchedulingType == UL_rtPS) {
                    ...
                    */
                    ...
                    int tmp_getpoll = con->getPOLL_interval();
                    if (req_slots_tmp1>0) {
                        allocationsize = ceil((sfqos->getMinReservedTrafficRate()*mac_->getFrameDuration())/8);
                        debug10 ("\tPoll_rtPS: No polling (bw-req>0), lastpoll :%d, period :%d\n", tmp_getpoll, sfqos->getPollingInterval() );
                        con->setPOLL_interval(0);
                    } else  {
                        if ( (tmp_getpoll / (sfqos->getPollingInterval() / mac_->getFrameDuration()) )>= 1 ) {
                    ...
                    debug10 ("UL.Check1.3.rtPS, MinReservedRate :%d, PRE-GRANT-SLOTS :%d, Peer-CID :%d, returnCID :%d, DIUC :%d, bw_req_header :%d, getBw :%d\n", sfqos->getMinReservedTrafficRate(), grant_slots, con->getPeerNode()->getBasic(OUT_CONNECTION)->get_cid(), return_cid_tmp, con->getPeerNode()->getDIUC(), GENERIC_HEADER_SIZE, con->getBw());
                ...
                if (ulGrantSchedulingType == UL_nrtPS) {
                    ...
                    if (req_slots_tmp1>0) {
                        allocationsize = (int) ceil((sfqos->getMinReservedTrafficRate()*mac_->getFrameDuration())/8);
                    } else  {
                        if (con->getCDMA() == 1) {
                    ...
                    debug10 ("UL.Check1.3.nrtPS, MinReservedRate :%d, PRE-GRANT-SLOTS :%d, Peer-CID :%d, returnCID :%d, DIUC :%d, getBw :%d\n", sfqos->getMinReservedTrafficRate(), grant_slots, con->getPeerNode()->getBasic(OUT_CONNECTION)->get_cid(), return_cid_tmp, con->getPeerNode()->getDIUC(), con->getBw());
...
int BSSchedulerStd::update_mcs_index(Ofdm_mod_rate current_rate, int current_mcs_index, bool increase)
{
...
bool BSSchedulerStd::check_modulation_change(Ofdm_mod_rate current_rate, int current_mcs_index, bool increase)
{
    int lower_mcs_bound = -1;
    int upper_mcs_bound = -1;
...
Ofdm_mod_rate BSSchedulerStd::change_rate(Ofdm_mod_rate rate, bool increase_modulation)
{
