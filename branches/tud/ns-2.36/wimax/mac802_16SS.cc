...
Mac802_16SS::Mac802_16SS() : Mac802_16 ()
{
    ...
    //ss_cqich_alloc_ie_indicator_ = false;
    set_cqich_alloc_ie_ind(false);

    CQICH_alloc_req_flag_ = FALSE;
    p_ffb_report_ = NULL;
    received_cqich_allocation_ie_ = FALSE;
    cqich_alloc_offset_ = 0;
    cqich_id_ = 0;
    ffb_report_duration_ = 0;
    ffb_report_period_ = 0;
    ffb_report_type_ = 0;
}
...
void Mac802_16SS::transmit(Packet *p)
{
    ...
    */ // rpi commented for OFDMA to allow multiple packets to be transmitted at the same time.
    ...
    debug2("Packet information: symbol_offset %d  symbol_num %d   subch_offset %d  subch_num %d\n",
           HDR_MAC802_16(p)->phy_info.OFDMSymbol_offset, HDR_MAC802_16(p)->phy_info.num_OFDMSymbol,
           HDR_MAC802_16(p)->phy_info.subchannel_offset,  HDR_MAC802_16(p)->phy_info.num_subchannels);
...
void Mac802_16SS::receive (Packet *pktRx_)
{
    ...
    if ( (wimaxHdr->phy_info.OFDMSymbol_offset == 0 && wimaxHdr->phy_info.num_OFDMSymbol == 0) || wimaxHdr->header.cid == BROADCAST_CID) { // this kind of packets are treated diff(OFDM types) basically it a bw req packet.
        debug2("profile of this burst is %d \n", wimaxHdr->phy_info.modulation_);
        if (ch->error()) {
            if (collision_) {
                ...
                debug2("\t 4 error in the packet, the Mac does not process");
                Packet::free(pktRx_);
            }
        ...
        if (ch->error()) {
            ...
            //   collision_ = false;
            // } else {
            debug2("\t 5 error in the packet, the Mac does not process");
    ...
    if (ch->ptype()==PT_MAC) {
        if (HDR_MAC802_16(pktRx_)->header.ht == 1 && HDR_MAC802_16(pktRx_)->header.ec ==1
                && HDR_MAC802_16(pktRx_)->phy_info.mcs_index_>=0 && amc_enable_ ==1) {
            debug2("SS %d received a MCS CQICH control message at CID %d with MCS index %d\n",
                   addr(), HDR_MAC802_16(pktRx_)->header.cid,HDR_MAC802_16(pktRx_)->phy_info.mcs_index_);
            free(pktRx_);
            pktRx_ = NULL;
            debug2("==============================SS receive End ===========================================\n");
            return;
    ...
    if (con!=NULL && con->getArqStatus () != NULL && con->getArqStatus ()->isArqEnabled() == 1 && con->isFragEnable () == true && con->isPackingEnable () == true && HDR_CMN(pktRx_)->ptype()!=PT_MAC) {
    ...
    if (HDR_CMN(pktRx_)->ptype()==PT_MAC) {
        ...
        mac_log(pktRx_);
        debug2("This is not a MAC packet,size is %d, type is %d, SS free it.\n", 8*ch->size(),HDR_CMN(pktRx_)->ptype());
        Packet::free(pktRx_);
    } else {
        ...
        if (state_ == MAC802_16_CONNECTED) {
            ...
            update_watch (&loss_watch_, 1);
            debug2("This is a MAC packet,size is %d,  MAC layer state is not right. SS drop it.\n", 8*ch->size());
            Packet::free(pktRx_);
            pktRx_=NULL;
            debug2("==============================SS receive End ===========================================\n");
            return;
...
void Mac802_16SS::process_ul_map (mac802_16_ul_map_frame *frame)
{
    ...
    debug2 ("At %f in %d, received UL_MAP for synch (substate=%d)\n",
    ...
    //debug2("SS cqich_alloc_ie_ind is %d\n",get_cqich_alloc_ie_ind());
    if (get_cqich_alloc_ie_ind() == true && amc_enable_ == 1) {
        debug2("SS [%d] Get an CQICH allocation IE.\n",addr());
        process_cqich_allocation_ie(frame);
        set_cqich_alloc_ie_ind(false);
    }
}


void Mac802_16SS::send_CQICH_alloc_request()
{
    hdr_mac802_16 *wimaxHdr;
    Packet* ffb_packet = getPacket ();
    wimaxHdr = HDR_MAC802_16(ffb_packet);
    PeerNode * peer = getPeerNode_head();

    wimaxHdr->header.ht = 1;
    wimaxHdr->header.ec = 0;
    wimaxHdr->header.cid = peer->getPrimary(IN_CONNECTION)->get_cid();

    wimaxHdr->phy_info.modulation_ = OFDM_BPSK_1_2;

    mac_hdr_cqich_alloc_request *req_tmp;
    req_tmp = reinterpret_cast<struct mac_hdr_cqich_alloc_request *>(& wimaxHdr->header);


    req_tmp->ht = 1;
    req_tmp->ec = 0;
    req_tmp->type = 7; //cqich allocation request
    req_tmp->fb_type = FFB_DL_AVERAGE_CINR; //DL Average SINR report
    req_tmp->preferred_period = 0; /*2^0 = 1, every frame*/
    req_tmp->cid = peer->getPrimary(IN_CONNECTION)->get_cid();

    peer->getPrimary(OUT_CONNECTION)->enqueue (ffb_packet);
    debug2("SS %d sends out the CQICH allocation request to BS.\r\n",addr());
}


double Mac802_16SS::get_effective_sinr()
{
    debug2("effective_SINR is %f \n", effective_sinr_);
    return (effective_sinr_ -INIT_SINR);
}


/*  To prepare such reports,statistics shall be quantized in 1 dB increments, ranging from a minimum of-10 dB (encoded 0x00)
to amaximum of 53 dB (encoded 0x3F). Values outside this range shall be assigned the closest extreme value within the scale.
*/
void Mac802_16SS::send_FastFeedback_Report()
{
    Packet *p;
    struct hdr_cmn *ch;
    struct hdr_mac802_16 *wimaxHdr;
    PeerNode * peer;
    peer = getPeerNode_head();

    if (peer == NULL) {
        debug2("In Send_FastFeedback_Report. The peer pointer or connection is empty.\n");
        return;
    }

    double get_sinr =  get_effective_sinr();
    if (get_sinr < 0) { // No updated SINR this time, do not send it.
        return;
    }



    /*save this FFB report.*/
    if (p_ffb_report_ == NULL) {
        p = getPacket();
        p_ffb_report_ = p;
    } else {
        p = p_ffb_report_;
    ...
    wimaxHdr = HDR_MAC802_16(p);
    p->allocdata (sizeof (struct FFB_report_ext));
    struct FFB_report_ext * ffb_ext = (struct FFB_report_ext*) p->accessdata();
    memset (ffb_ext, 0, sizeof (struct FFB_report_ext));

    struct mac_hdr_ffb_report *p_report_hdr = (struct mac_hdr_ffb_report *)&wimaxHdr->header;
    p_report_hdr->fb_type = ffb_report_type_;
    p_report_hdr->ht = 1;
    p_report_hdr->ec = 1;
    p_report_hdr->cii = 1;
    p_report_hdr->type = 0; //Feedback header

    switch (ffb_report_type_) {
    case FFB_MIMO_PRECODING:
    case FFB_MIMO_MODE:
    case FFB_UEP:
    case FFB_BAND_AMC_CINR:
    case FFB_INDICATION_FLAG:
        printf("BS ask for other kinds of report. \r\n");
        break;

    case FFB_DL_AVERAGE_CINR: {
        debug2("before 2 SS report SINR is %f, BLER is %f\n",get_sinr, get_current_bler());
        double report_sinr = get_sinr;
        if (report_sinr>=MIN_SINR && report_sinr < MAX_SINR) {
            if (get_channel_error() == FALSE)
                p_report_hdr->fb_content = get_sinr;
            else
                p_report_hdr->fb_content = MIN_SINR;
        } else if (report_sinr <MIN_SINR) {
            p_report_hdr->fb_content = MIN_SINR;
        } else {
            if (get_channel_error() == FALSE)
                p_report_hdr->fb_content = MAX_SINR;
            else
                p_report_hdr->fb_content = MIN_SINR;
        }
    }
    ffb_ext->bler = get_current_bler();
    ffb_ext->mcs_index = get_ss_current_mcs_index();
    ...
    default:
        printf("[Error] Unknown feedback type.\r\n");
        break;
    }


    /*	if( NULL != peer->getPrimary(IN_CONNECTION))
    	{
    		p_report_hdr->cid =peer->getPrimary(IN_CONNECTION)->get_cid();
    		debug2("ffb report incoming cid[%d]\r\n", p_report_hdr->cid);
    	}
    	if( NULL!= peer->getPrimary(OUT_CONNECTION))
    	{
    		peer->getPrimary(OUT_CONNECTION)->enqueue(p);
    	}*/

    debug2("Finished sending FFB report.\r\n");
}


void Mac802_16SS::ffb_process()
{
    double fduration = getFrameDuration();
    double timerDelay  = fduration * pow(2,ffb_report_period_);
    //debug2("ffb report period %d\n", ffb_report_period_);
    //debug2("frame duration is [%e]\t timerdelay[%e]\r\n", fduration, timerDelay);

    //report immediately so that supports report once.
    send_FastFeedback_Report();

    if (ffb_report_duration_ == FFB_ENDLESS_REPORT) {
        debug2("Endless timer arrange.\n");
        ffbTimer_ = new WimaxFfbTimer(this);
        ffbTimer_->resched(timerDelay);
    } else if (ffb_report_duration_ == FFB_REPORT_DEALLOCATION) {
        debug2("Deallocation timer.\n");
        ffbTimer_->cancel();
        delete ffbTimer_;
    } else if (ffb_report_duration_ == FFB_REPORT_ONE_TIME) {
        debug2("FFB report only once.\n");
        ffbTimer_->cancel();
        delete ffbTimer_;
    }
}
...
void Mac802_16SS::process_cqich_allocation_ie(mac802_16_ul_map_frame *frame)
{
    /*Here I activate the FFB timer to enable ffb report.*/
    int nbies = frame->nb_ies;
    mac802_16_ulmap_ie *ies = frame->ies;

    for (int i = 0 ;  i < getMap()->getUlSubframe()->getNbPdu() ; i++) {
        ...
        if (( ies[i].uiuc == UIUC_EXT_UIUC ) && ies[i].extended_uiuc == UIUC_CQICH_ALLOCATION_IE
            ...
            debug2("Starting the CQICH fast feedback timer.\n");

            ffb_report_period_ = ies[i].cqich_alloc_ie.period;
            ffb_report_duration_ = ies[i].cqich_alloc_ie.duration;
            ffb_report_type_ = ies[i].cqich_alloc_ie.fb_type;
            cqich_id_ = ies[i].cqich_alloc_ie.cqich_id;
            cqich_alloc_offset_ = ies[i].cqich_alloc_ie.alloc_offset;
            debug2("in SS side, the allocated CQICH_id is %d\t  CQICH_Slot_offset is %d\n", cqich_id_, cqich_alloc_offset_);

            if (cqich_id_ == 0) { //error allcoation of cqich_id. No free CQICH slot can be scheduled in BS side.
                debug2("Warning: No CQICH Slot at BS side can be allocated. No FFB report in this SS.\n");
            } else {
                ffb_process();
                received_cqich_allocation_ie_ = TRUE;
            }
        }
    }
}
...
void Mac802_16SS::init_ranging ()
{
    ...
    for (PhyPdu *p = getMap()->getUlSubframe ()->getFirstPdu(); p ; p= p ->next_entry()) {
        ...
        if (b->getIUC() == UIUC_EXT_UIUC &&
            ...
            // frame->req_ul_burst_profile = default_uiuc_ & 0xF; //we use lower bits only
    ...
    for (PhyPdu *pdu = ulsubframe->getFirstPdu(); pdu ; pdu = pdu->next_entry()) {
        if (pdu->getBurst(0)->getIUC()==UIUC_INITIAL_RANGING) {
            ...
            */
            ...
            // vr@tud
            int result = Random::integer(pow (2, s_window)+1);
			...
			*/
            ...
            u_char header_code = u_char( Random::uniform( macmib_.cdma_code_init_start, macmib_.cdma_code_init_stop + 1));
...
void Mac802_16SS::send_registration ()
{
    ...
    peer->getPrimary(OUT_CONNECTION)->enqueue (p);
    debug2(" SS %d sending registration\n", addr());
...
//      upsecondary->initCDMA_SSID();
    ...
    /*After registration procedure, SS starts to ask for CQICH setup and feedback request.*/
    if (amc_enable_ == 1 && CQICH_alloc_req_flag_!= TRUE) {
        send_CQICH_alloc_request();
        CQICH_alloc_req_flag_ = TRUE;
    }
}
...
{
    debug2("SS lost synchronization.\n");
#ifdef USE_802_21
    int poa = -1;
...
int  Mac802_16SS::GetInitialChannel()
{
    ...
    int Rand_Num = int(Random::uniform( 1, 500));
    return Rand_Num;
}
