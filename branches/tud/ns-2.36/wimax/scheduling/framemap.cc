...
Packet* FrameMap::getUL_MAP( )
{
    ...
    for (PhyPdu *p = ulsubframe_.getFirstPdu(); p ; p= p ->next_entry()) {
        ...
        ies[i].isCqichSlot = b->getCqichSlotFlag();  // pass the CQICH slot indication info to the SS.
        //debug2("In Burst, UIUC %d    CqichSlotFlag is %d  ies[i].isCqichSlot is %d\n", b->getIUC(), b->getCqichSlotFlag(), ies[i].isCqichSlot);
        ies[i].cqich_alloc_ie.ss_mac_id = b->getBrustSsMacId();
        ies[i].cqich_alloc_ie.cqich_id = b->get_cqich_id();
        ...
        //debug2("uiuc is %d    ext_uiuc is %d\n", b->getIUC() ,  b->getExtendedUIUC());

        /*passed by bsscheduler::schedule(), once the burst has the information of CQICH allocation IE,
        	then in getUL_MAP(), a UL_MAP with CQICH_Alloc_IE info will be made to let framemap to actually
        	generate a UL-MAP to send this infor to the specific SS.*/
        if (b->getIUC() == UIUC_EXT_UIUC  &&  b->getExtendedUIUC ()== UIUC_CQICH_ALLOCATION_IE) {
            debug2("in getUL_MAP. bsscheduler allocate for cqich Alloc IE.\n");
            ies[i].uiuc =  UIUC_EXT_UIUC;
            ies[i].extended_uiuc = UIUC_CQICH_ALLOCATION_IE;
            ies[i].cqich_alloc_ie.ext_uiuc = UIUC_CQICH_ALLOCATION_IE;
            ies[i].cqich_alloc_ie.cqich_id = b->get_cqich_id();
            ies[i].cqich_alloc_ie.alloc_offset = b->get_cqich_alloc_offset();
            ies[i].cqich_alloc_ie.period = b->get_cqich_period();
            ies[i].cqich_alloc_ie.duration = b->get_cqich_duration();
            ies[i].cqich_alloc_ie.fb_type = b->get_cqich_fb_type();
        }
    ...
    debug2 ("Frame_map.UL_MAP_IE index :%d, #normal_ie :%d, #cdma_ie :%d, ie_size :%f, ul_map_size :%d\n", i, i_normal_ie, i_cdma_ie, sum_ie_size, int(ceil(GET_UL_MAP_SIZE(0)+sum_ie_size)));
    ch->size() += int(ceil(GET_UL_MAP_SIZE(0) + sum_ie_size));
    return p;
}
...
{
    // Clear previous information
    debug2("ParseULMAPframe() parsing UL-MAP frame \n");
    ...
    for (int i = 0 ; i < nbies ; i++) {
        ...
        if (b->getIUC()!=UIUC_END_OF_MAP) {
            ...
            debug2("UIUC %d\t isCqichSlot %d\n", ies[i].uiuc, ies[i].isCqichSlot);
            if ((ies[i].uiuc == UIUC_PROFILE_2) && (ies[i].isCqichSlot== TRUE)) {
                debug2("Find a CQICH Slot.  ss_mac_id in CQICH_alloc_ie is %d\n",ies[i].cqich_alloc_ie.ss_mac_id );
                b->setCqichSlotFlag(TRUE);
                int cqich_id = ies[i].cqich_alloc_ie.cqich_id;
                b->set_cqich_id(cqich_id);
                b->setBurstSsMacId( ies[i].cqich_alloc_ie.ss_mac_id);
            }

            debug2("parse UL-MAP. uiuc=%d  ext_uiuc=%d  cqich_ext_uiuc=%d\n",ies[i].uiuc,  ies[i].extended_uiuc, ies[i].cqich_alloc_ie.ext_uiuc );
            if (ies[i].extended_uiuc == UIUC_CQICH_ALLOCATION_IE &&  ies[i].cqich_alloc_ie.ext_uiuc == UIUC_CQICH_ALLOCATION_IE
                    && (ies[i].cqich_alloc_ie.ss_mac_id == mac_->addr())) {
                debug2("In FrameMap parseULMAP:  ss_mac_id in CQICH_ALLOC_IE %d\t  local MAC_ID %d\n",ies[i].cqich_alloc_ie.ss_mac_id,mac_->addr());
                debug2("In FrameMap, there is a CQICH allocation IE in UL-MAP.\n");
                mac_->set_cqich_alloc_ie_ind(true);
            }
        }
    }
    debug2("End of parseUlMAPframe().\n");
}
...
void FrameMap::print_frame ()
{
    ...
    for (PhyPdu *p = ulsubframe_.getFirstPdu(); p ; p= p ->next_entry()) {
        ...
        case UIUC_PROFILE_7:
        case UIUC_PROFILE_8:
            if (b->getCqichSlotFlag() == TRUE) {
                printf("\tCQICH Slot Tx Region.\n");
            }
