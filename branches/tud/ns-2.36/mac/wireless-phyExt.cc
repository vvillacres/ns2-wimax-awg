...
void WirelessPhyExt::sendDown(Packet *p) {
	...
	switch (state) {
	case TXing:
		std::cout << "wrong in MAC logic case 1"<<std::endl;
		exit(1);
		break;
	case RXing:
		rX_Timer.cancel();
...
void WirelessPhyExt::handle_PreRXtimeout() {
	...
	//	std::cout<<"sinr"<<SINR_Th_RX<<std::endl;
	if (powerMonitor->SINR(power_RX) < SINR_Th_RX) {
		if (PHY_DBG)
...
void WirelessPhyExt::log(char * event, char* additional) {
	if (PHY_DBG)
		std::cout<<"L "<<Scheduler::instance().clock()<<" "<<node_->nodeid()<<" "<<"PHY"<<" "<<event<<" "
				<<additional<<std::endl;
}
...
double PowerMonitor::SINR(double Pr) {
	if (getPowerLevel()-Pr<=0) {
		//			std::cout<<"PowerLevel lower than Pr"<<std::endl;exit(-1);
		//			char msg[1000];
