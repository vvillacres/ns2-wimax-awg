 ...
 */
int frame_number_bs26=0;

//This structure is used for virtual allocation and "indexBurst" is used for #counting bursts
int indexBurst=0;
struct virtual_burst {
    ...
    int dl_ul;    //0 = DL, 1 = UL
    int ie_type;
} virtual_allocation[MAX_MAP_IE], ul_virtual_allocation[MAX_MAP_IE];


static class BSScheduler26Class : public TclClass
{
public:
    BSScheduler26Class() : TclClass("WimaxScheduler/BS26") {}
    TclObject* create(int, const char*const*) {
        return (new BSScheduler26());
 ...
 * Create a scheduler
 */
BSScheduler26::BSScheduler26 () : WimaxScheduler ()
{
 ...
 * @return The MAC casted to BSScheduler
 */
Mac802_16BS* BSScheduler26::getMac()
{
 ...
 */
int BSScheduler26::command(int argc, const char*const* argv)
{
    if (argc == 3) {
 ...
 */
int BSScheduler26::getBWopportunity ()
{
 ...
 */
int BSScheduler26::getInitRangingopportunity ()
{
...
int BSScheduler26::increase_dl_map_ie(int num_of_entries, int totalslots, int num_ie)
{
...
//padding to byte based
    float dl_map_increase_byte = virtual_allocation[0].byte + num_ie*dl_ie_size;
    int dl_map_increase_slots = virtual_allocation[0].rep*(int)ceil(ceil(dl_map_increase_byte)/(double)phy->getSlotCapacity(virtual_allocation[0].mod,DL_));

    debug10 ("Check_DLMAP.From array DL MAP byte :%f, slots :%d, IE_SIZE :%f, slotcapacity :%d, #ie :%d\n", virtual_allocation[0].byte, virtual_allocation[0].numslots, dl_ie_size, phy->getSlotCapacity(virtual_allocation[0].mod,DL_), num_ie);
    debug10 ("\tNew DL MAP byte :%f, slots :%d\n", dl_map_increase_byte, dl_map_increase_slots);

    for (int i = 1; i<num_of_entries; i++) {
        debug2 ("\t index :%d, numslots :%d, accum :%d\n", i, virtual_allocation[i].numslots, slots_sofar);
        slots_sofar = slots_sofar + virtual_allocation[i].numslots;
    }

    int slots_temp = slots_sofar;
    slots_sofar = slots_sofar + dl_map_increase_slots;
    if ( slots_sofar <= totalslots) {
        debug10 ("Check_DLMAP. Old DL map byte :%f, slots :%d, New DL map byte :%f, slots :%d, #ie :%d\n", virtual_allocation[0].byte, virtual_allocation[0].numslots, dl_map_increase_byte, dl_map_increase_slots, num_ie);

        virtual_allocation[0].byte = dl_map_increase_byte;
        virtual_allocation[0].numslots = dl_map_increase_slots;
        return slots_sofar;
    } else {
        return -1;
    }
...
int BSScheduler26::overallocation_withdlmap(int num_of_entries, int totalslots, int ownslots)
{
    ...
    float dl_ie_size = DL_MAP_IE_SIZE;
//padding to byte based
    float dl_map_increase_byte = virtual_allocation[0].byte + dl_ie_size;
    int dl_map_increase_slots = virtual_allocation[0].rep*(int)ceil((ceil)(dl_map_increase_byte)/(double)phy->getSlotCapacity(virtual_allocation[0].mod,DL_));

    debug10 ("Check_DLMAP.From array DL MAP byte :%f, slots :%d, IE_SIZE :%f, slotcapacity :%d\n", virtual_allocation[0].byte, virtual_allocation[0].numslots, dl_ie_size, phy->getSlotCapacity(virtual_allocation[0].mod,DL_));
    debug10 ("\tNew DL MAP byte :%f, slots :%d\n", dl_map_increase_byte, dl_map_increase_slots);
    ...
    if ( slots_sofar <= totalslots) {
        debug10 ("Check_DLMAP. Old DL map byte :%f, slots :%d, New DL map byte :%f, slots :%d, available burst slot size :%d\n", virtual_allocation[0].byte, virtual_allocation[0].numslots, dl_map_increase_byte, dl_map_increase_slots, ownslots);
        ...
        if ( (slots_temp + dl_map_increase_slots) <= totalslots ) {
            debug10 ("Check_DLMAP. Old DL map byte :%f, slots :%d, New DL map byte :%f, slots :%d, available slot burst size :%d\n", virtual_allocation[0].byte, virtual_allocation[0].numslots, dl_map_increase_byte, dl_map_increase_slots, totalslots - slots_temp - dl_map_increase_slots);
            virtual_allocation[0].byte = dl_map_increase_byte;
            virtual_allocation[0].numslots = dl_map_increase_slots;
...
int BSScheduler26::max_conn_withdlmap(int num_of_entries, int totalslots)
{
    ...
    float virtual_increase_byte = virtual_allocation[0].byte;
    int virtual_increase_slots = virtual_allocation[0].numslots;

    for (int i = 0; i<num_of_entries; i++) {
//	debug2 ("\t index :%d, numslots :%d, accum :%d\n", i, virtual_allocation[i].numslots, slots_sofar);
        slots_sofar = slots_sofar + virtual_allocation[i].numslots;
    }
    int slots_data = slots_sofar - virtual_allocation[0].numslots;

    debug10 ("Check_DLMAP. Max supported conn total_dl_slots :%d, sofar slots :%d, dlmap slots :%d, dlmap byte :%f, available slots :%d\n", totalslots, slots_sofar, virtual_allocation[0].numslots, virtual_allocation[0].byte, totalslots-slots_sofar);

    while (1) {
        virtual_increase_byte += dl_ie_size;
        int virtual_increase_byte_ceil = (int)ceil(virtual_increase_byte);
        virtual_increase_slots = virtual_allocation[0].rep*ceil((double)(virtual_increase_byte_ceil)/(double)phy->getSlotCapacity(virtual_allocation[0].mod,DL_));
...
int BSScheduler26::freeslots_withdlmap_given_conn(int num_of_entries, int totalslots, int newconn)
{
...
//	debug2 ("\t index :%d, numslots :%d, accum :%d\n", i, virtual_allocation[i].numslots, slots_sofar);
    ...
    debug10 ("Check_DLMAP. #Free slots newconn :%d, sofar slots :%d, dlmap slots :%d, dlmap byte :%f, newdlmap slots :%d, newdlmap byte :%d\n", newconn, slots_sofar, virtual_allocation[0].numslots, virtual_allocation[0].byte, virtual_increase_slots, virtual_increase_byte_ceil);
...
int BSScheduler26::overallocation_withoutdlmap(int num_of_entries, int totalslots, int ownslots)
{
...
void BSScheduler26::bubble_sort (int arrayLength, con_data_alloc array[], int sort_field)
{
    int i, j, flag = 1;
    con_data_alloc temp;
...
int BSScheduler26::doesvirtual_allocexist(int num_of_entries, int cid)
{
    for (int i = 0; i<num_of_entries-1; i++) {
        if (virtual_allocation[i].cid == cid) {
            return cid;
        }
    }
    return -1;
...
int BSScheduler26::addslots_withoutdlmap(int num_of_entries, int byte, int slots, int cid)
{
    for (int i = 0; i<num_of_entries-1; i++) {
        if (virtual_allocation[i].cid == cid) {
            virtual_allocation[i].byte += byte;
            virtual_allocation[i].numslots += slots;
            return i;
        }
    }
    return -1;
...
int BSScheduler26::check_overallocation(int num_of_entries)
{
 ...
 */
void BSScheduler26::schedule ()
{
    debug2("\n\n======================BSScheduler26::schedule () Begin ============================\n");
...
/////////////////////
    ...
    bzero(virtual_allocation, MAX_MAP_IE*sizeof(virtual_burst));
    indexBurst = 0;

//1. Virtual DL_MAP
//Note that, we virtually allocate all burst into "virtual_allocation[]", then we will physically allocate/map those allocation into burst/physical later. The reason is because we have variable part of DL_MAPs
//"indexBurst" is the #total_burst including each DL_MAP, UL_MAP, DCD, UCD, other broadcast message and each CID allocation
//In this version, the allocation is per CID/burst for downlink
    ...
    virtual_allocation[indexBurst].alloc_type = 0;
    virtual_allocation[indexBurst].cid = BROADCAST_CID;
    virtual_allocation[indexBurst].n_cid = 0;
    virtual_allocation[indexBurst].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
    virtual_allocation[indexBurst].preamble = true;
    virtual_allocation[indexBurst].numslots = fixed_dl_map_slots;
    virtual_allocation[indexBurst].mod = fixed_dl_map_mod;
    virtual_allocation[indexBurst].byte = fixed_byte_dl_map;
    virtual_allocation[indexBurst].rep =  Repetition_code_;
    virtual_allocation[indexBurst].dl_ul = 0;
    virtual_allocation[indexBurst].ie_type = 0;
    indexBurst++;
...
//2. Virtual UL_MAP:
    ...
    virtual_allocation[indexBurst].alloc_type = 1;
    virtual_allocation[indexBurst].cid = BROADCAST_CID;
    virtual_allocation[indexBurst].n_cid = 0;
    virtual_allocation[indexBurst].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
    virtual_allocation[indexBurst].preamble = true;
    virtual_allocation[indexBurst].numslots = fixed_ul_map_slots;
    virtual_allocation[indexBurst].mod = fixed_ul_map_mod;
    virtual_allocation[indexBurst].byte = fixed_byte_ul_map;
    virtual_allocation[indexBurst].rep =  Repetition_code_;
    virtual_allocation[indexBurst].dl_ul = 0;
    virtual_allocation[indexBurst].ie_type = 0;
    int add_ie_to_dlmap = increase_dl_map_ie(indexBurst, total_dl_slots_pusc, 1);
...
//3. Virtual DCD
    if (getMac()->isSendDCD() || map->getDlSubframe()->getCCC()!= getMac()->getDlCCC()) {
        ...
        virtual_allocation[indexBurst].alloc_type = 2;
        virtual_allocation[indexBurst].cid = BROADCAST_CID;
        virtual_allocation[indexBurst].n_cid = 1;
        virtual_allocation[indexBurst].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
        virtual_allocation[indexBurst].preamble = false;
        virtual_allocation[indexBurst].numslots = dcd_slots;
        virtual_allocation[indexBurst].mod = dcd_mod;
        virtual_allocation[indexBurst].byte = byte_dcd;
        virtual_allocation[indexBurst].rep =  rep_dcd;
        virtual_allocation[indexBurst].dl_ul = 0;
        virtual_allocation[indexBurst].ie_type = 0;
        int add_ie_to_dlmap = increase_dl_map_ie(indexBurst, total_dl_slots_pusc, 1);
        if (add_ie_to_dlmap <0 ) {
...
//4. Virtual UCD
    if (getMac()->isSendUCD()|| map->getUlSubframe()->getCCC()!= getMac()->getUlCCC()) {
        ...
        virtual_allocation[indexBurst].alloc_type = 3;
        virtual_allocation[indexBurst].cid = BROADCAST_CID;
        virtual_allocation[indexBurst].n_cid = 1;
        virtual_allocation[indexBurst].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
        virtual_allocation[indexBurst].preamble = false;
        virtual_allocation[indexBurst].numslots = ucd_slots;
        virtual_allocation[indexBurst].mod = ucd_mod;
        virtual_allocation[indexBurst].byte = byte_ucd;
        virtual_allocation[indexBurst].rep =  rep_ucd;
        virtual_allocation[indexBurst].dl_ul = 0;
        virtual_allocation[indexBurst].ie_type = 0;
        int add_ie_to_dlmap = increase_dl_map_ie(indexBurst, total_dl_slots_pusc, 1);
        if (add_ie_to_dlmap <0 ) {
    ...
    int check_dl_slots = check_overallocation(indexBurst);

    debug10 ("DL.broadcast messages (slots): (v)Fixed_DL_MAP :%d, (v)UL_MAP :%d, DCD :%d, UCD :%d, MaxDL :%d\n", virtual_allocation[0].numslots, virtual_allocation[1].numslots, virtual_allocation[2].numslots, virtual_allocation[3].numslots, total_dl_slots_pusc);
    debug10 ("\t(bytes): (v)Fixed_DL_MAP :%f, (v)UL_MAP :%f, DCD :%f, UCD :%f\n", virtual_allocation[0].byte, virtual_allocation[1].byte, virtual_allocation[2].byte, virtual_allocation[3].byte);
...
//5. Virtual Other broadcast
    if (mac_->getCManager()->get_connection (BROADCAST_CID, OUT_CONNECTION)->queueByteLength()>0) {
        ...
        virtual_allocation[indexBurst].alloc_type = 4;
        virtual_allocation[indexBurst].cid = BROADCAST_CID;
        virtual_allocation[indexBurst].n_cid = 1;
        virtual_allocation[indexBurst].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
        virtual_allocation[indexBurst].preamble = false;
        virtual_allocation[indexBurst].numslots = bc_slots;
        virtual_allocation[indexBurst].byte = real_bytes;
        virtual_allocation[indexBurst].rep =  1;
        virtual_allocation[indexBurst].dl_ul = 0;
        virtual_allocation[indexBurst].ie_type = 0;
        int add_ie_to_dlmap = increase_dl_map_ie(indexBurst, total_dl_slots_pusc, 1);
        if (add_ie_to_dlmap <0 ) {
        ...
        debug10 ("DL.other_broadcast messages (slots): %d, (bytes): %f\n", virtual_allocation[indexBurst].numslots, virtual_allocation[indexBurst].byte);
        indexBurst++;
    }
//End of Virtual allocation for all broadcast message

    int indexBurst_before_data = indexBurst;

    int check_dl_duration = ceil((double)check_overallocation(indexBurst)/(double)total_dl_subchannel_pusc);
    ...
    for (int i=0; i<indexBurst; i++) {
        db = (DlBurst*) map->getDlSubframe()->getPdu ()->addBurst (nbdlbursts++);
        db->setCid (virtual_allocation[i].cid);
        db->setIUC (virtual_allocation[i].iuc);
        db->setPreamble(virtual_allocation[i].preamble);
        db->setStarttime(virtual_symbol_offset);
        db->setSubchannelOffset(virtual_subchannel_offset);

        int num_of_subchannel = virtual_allocation[i].numslots;
        ...
        virtual_accum_slots += virtual_allocation[i].numslots;

        debug10("DL.add bursts: index :%d, cid :%d, iuc :%d, subchannel_offset :%d, symbol_offset :%d, num_slots (num_subchannel) :%d, num_symbol :%d, accum_slots :%d\n", i, virtual_allocation[i].cid, virtual_allocation[i].iuc, virtual_subchannel_offset, virtual_symbol_offset, virtual_allocation[i].numslots, num_of_symbol, virtual_accum_slots);
...
/*       // rpi removed this dl burst ----------------------------------------------------------------------------
	 int BSScheduler26::addDlBurst (int burstid, Connection *c, int iuc, int dlduration, int maxdlduration)
	 {
	 double txtime; //tx time for some data (in second)
	 int txtime_s;  //number of symbols used to transmit the data
 ...
 * @param the new allocation status
 */
void BSScheduler26::addDlBurst (int burstid, int cid, int iuc, int ofdmsymboloffset, int numofdmsymbols, int subchanneloffset, int numsubchannels)
{
    DlBurst *db = (DlBurst*) mac_->getMap()->getDlSubframe()->getPdu ()->addBurst (burstid);
    db->setCid (cid);
    db->setIUC (iuc);
...
int BSScheduler26::doesMapExist(int cid, int *cid_list, int num_of_entries)
{
    for (int i = 0; i<num_of_entries; ++i) {
        if (cid_list[i]==cid)
            return i;
...
mac802_16_dl_map_frame * BSScheduler26::dl_stage2(Connection *head, int input_subchannel_offset, int total_subchannels, int total_symbols, int symbol_start, int stripping, int total_dl_slots_pusc)
{
    ...
    int temp_indexBurst = indexBurst;
        ...
        while (con!=NULL) {
            if (con->get_category() == contype) {
                ...
                if ( con_byte > 0 ) {
                    ...
                    int virtual_allocation_exist = doesvirtual_allocexist(indexBurst, con->get_cid());
                    int add_slots = 0;
                    if (virtual_allocation_exist > 0) {
                        int add_slots = overallocation_withoutdlmap (indexBurst, total_dl_slots_pusc, con_slots);
                        int t_index = addslots_withoutdlmap(indexBurst, con_byte, con_slots, con->get_cid());
                        debug10 ("DL Add more slots into existing burst contype(%d), index :%d, cid :%d, byte :%f, slots :%d\n", contype, t_index, con->get_cid(), virtual_allocation[t_index].byte, virtual_allocation[t_index].numslots);
                    } else {
                        int add_slots = overallocation_withdlmap (indexBurst, total_dl_slots_pusc, con_slots);
                        if (add_slots > 0) {
                            virtual_allocation[indexBurst].alloc_type = 5;
                            virtual_allocation[indexBurst].cid = con->get_cid();
                            virtual_allocation[indexBurst].n_cid = 1;
                            //virtual_allocation[indexBurst].iuc = getDIUCProfile(con_mod);
                            virtual_allocation[indexBurst].iuc = getDIUCProfile(OFDM_QPSK_1_2);
                            virtual_allocation[indexBurst].numslots = add_slots;
                            virtual_allocation[indexBurst].byte = (add_slots * mac_->getPhy()->getSlotCapacity(con_mod, DL_));
                            virtual_allocation[indexBurst].rep =  rep;
                            virtual_allocation[indexBurst].dl_ul = 0;
                            virtual_allocation[indexBurst].ie_type = 0;
                            debug10 ("DL Add new burst contype(%d), index :%d, cid :%d, byte :%f, slots :%d\n", contype, indexBurst, con->get_cid(), virtual_allocation[indexBurst].byte, virtual_allocation[indexBurst].numslots);
                            indexBurst++;
                        }
                    }
                }//end if q > 0

                debug10 ("DL2.management msg, contype(%d), index :%d, CID :%d, Q-Bytes :%d, allocationsize :%d\n", contype, indexBurst, con->get_cid(), con->queueByteLength(), con_byte);
            }
            con = con->next_entry();
        }//end con!=null
    }//end for loop
...
//End ARQ aware scheduling
                    ...
                    if (virtual_allocation_exist > 0) {
                        int add_slots = overallocation_withoutdlmap (indexBurst, total_dl_slots_pusc, con_slots);
                        addslots_withoutdlmap(indexBurst, con_byte, con_slots, con->get_cid());
                    } else {
                        int add_slots = overallocation_withdlmap (indexBurst, total_dl_slots_pusc, con_slots);
                        if (add_slots > 0) {
                            virtual_allocation[indexBurst].alloc_type = 5;
...
//Find out how many connections can be supported given variable DL MAP
    int max_conn_dlmap = max_conn_withdlmap(indexBurst, total_dl_slots_pusc);
...
//Sorting and make sure #request slots is correct
    ...
    if (j_need > 0 ) t_freeslots = freeslots_withdlmap_given_conn(indexBurst, total_dl_slots_pusc, j_need);
...
//Assign granted slots to each connection (virtual burst)
    for (int i=0; i<j_need; i++) {
        if (con_data[i].grant_slots >=0 ) {
            ...
            int virtual_allocation_exist = doesvirtual_allocexist(indexBurst, con_data[i].cid);
            int add_slots = 0;
            if (virtual_allocation_exist > 0) {
                int add_slots = overallocation_withoutdlmap (indexBurst, total_dl_slots_pusc, con_slots);
                addslots_withoutdlmap(indexBurst, con_byte, con_slots, con_data[i].cid);
            } else {
                int add_slots = overallocation_withdlmap (indexBurst, total_dl_slots_pusc, con_slots);

                if (add_slots > 0) {
                    virtual_allocation[indexBurst].alloc_type = 5;
                    virtual_allocation[indexBurst].cid = con_data[i].cid;
                    virtual_allocation[indexBurst].n_cid = 1;
                    virtual_allocation[indexBurst].iuc = getDIUCProfile(con_mod);
                    virtual_allocation[indexBurst].numslots = add_slots;
                    virtual_allocation[indexBurst].byte = (add_slots * mac_->getPhy()->getSlotCapacity(con_mod, DL_));
    ...
    int total_slots_sofar = check_overallocation(temp_indexBurst);
    ...
    ie_index = indexBurst - temp_indexBurst;
 ...
 * Added by Ritun
 */
diuc_t BSScheduler26::getDIUCProfile(Ofdm_mod_rate rate)
{
    Profile *p;
...
/* Added by Ritun
 */
uiuc_t BSScheduler26::getUIUCProfile(Ofdm_mod_rate rate)
{
    Profile *p;
...
struct mac802_16_ul_map_frame * BSScheduler26::ul_stage2(Connection *head, int total_subchannels, int total_symbols, int symbol_start, int stripping) {
...
int BSScheduler26::update_mcs_index(Ofdm_mod_rate current_rate, int current_mcs_index, bool increase)
{
...
bool BSScheduler26::check_modulation_change(Ofdm_mod_rate current_rate, int current_mcs_index, bool increase)
{
    int lower_mcs_bound = -1;
    int upper_mcs_bound = -1;
...
Ofdm_mod_rate BSScheduler26::change_rate(Ofdm_mod_rate rate, bool increase_modulation)
{
