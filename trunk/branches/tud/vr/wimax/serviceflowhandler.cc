...
#define DEFAULT_POLLING_INTERVAL 1000 // in millisecond
 ...
 */
void ServiceFlowHandler::removeFlow (int sfid)
{
...
void ServiceFlowHandler::processDSA_req (Packet *p)
{
    ...
    if (mac_->getNodeType()==STA_MN) {
        ...
        */
        ...
        if (data->get_serviceflow ()->getQoS ()->isArqEnabled () == 1 ) {
            debug2("ARQ status is enable.\n");
            arqstatus = new Arqstatus ();
            data->setArqStatus (arqstatus);
            data->getArqStatus ()->setArqEnabled (data->get_serviceflow ()->getQoS ()->isArqEnabled ());
    ...
    if (mac_->getNodeType()==STA_MN) {
        ...
        */
        ...
        if (dsa_rsp_frame->uplink) {
            ...
            debug2("set outcoming data connection for mac %d\n", mac_->addr());
//Begin RPI
            if (data->get_serviceflow ()->getQoS ()->isArqEnabled () == TRUE ) {
                //schedule timer based on retransmission time setting
...
int ServiceFlowHandler::addStaticFlow (int argc, const char*const* argv)
{
	...
	if ( maxSustainedTrafficRate <= 0) {
		...
		maxSustainedTrafficRate = UINT_MAX;
	}
	...
	if ( minReservedTrafficRate > maxSustainedTrafficRate ) {
		...
		fprintf(stderr, "Minimum Revered Traffic Rate can be greater than the Maximum Sustained Traffic Rate \n");
	}
    ...
    arqBlockSize = u_int16_t( atoi( argv[15]));

	double frameDuration = mac_->getFrameDuration() * 1e3; // FrameDuration is in seconds
	// only for uplink connections
	if ( direction == UL ) {
		if (( ulGrantSchedulingType == UL_UGS) or ( ulGrantSchedulingType == UL_ertPS) ) {
			unsolicitedGrantInterval = u_int16_t( atof( argv[16]));
			...
			unsolicitedPollingInterval = u_int16_t( atof( argv[16])); // IEEE 802.16-2009 page 372
			...
			unsolicitedPollingInterval = u_int16_t( DEFAULT_POLLING_INTERVAL); // default for nrtPS (one second or less)
		}
	}

	timeBase = u_int16_t( atof( argv[17]));
	if ( timeBase < frameDuration ) {
		timeBase = u_int16_t( frameDuration);
	}

	packetErrorRate = double( atof( argv[18]));
