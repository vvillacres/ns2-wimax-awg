...
#include "scheduling/bsschedulerstd.h"
...
Mac802_16BS::Mac802_16BS() : Mac802_16 (), cl_head_(0), cl_tail_(0), ctrlagent_(0)
{
    ...
    set_cqich_alloc_ie_ind( false);
    bzero(cqich_alloc_info_buffer,sizeof(struct bs_cqich_alloc_ie_info)*MAX_SYNC_CQICH_ALLOC_INFO);
...
void Mac802_16BS::init ()
{
    ...
    /*Init AMC related Params.*/
    for (int i=0; i< MAX_NUM_SS_IN_ONE_CELL; ++i) {
        set_change_modulation_flag(i, false);
        set_registered_flag(i,false);
        set_increase_modulation(i, false);
        set_smooth_factor(i, 0);
        set_change_ul_modulation_flag(i, false);
        set_increase_ul_modulation(i, TRUE);
        set_ul_smooth_factor(i, 0);
    }
}
...
void Mac802_16BS::transmit(Packet *p)
{
    ...
    debug2("\n\n==============================BS Transmit begin ===========================================\n");
    debug2 ("At %f (BS) in Mac %d, sending packet, TRANSMIT, (type :%s, size :%d, txtime :%.9f)\n", NOW, index_, packet_info.name(ch->ptype()), ch->size(), ch->txtime());
    if (ch->ptype()==PT_MAC) {
...
void Mac802_16BS::sendUp (Packet *p)
{
    ...
    struct mac_hdr_ffb_report *p_report_hdr = (struct mac_hdr_ffb_report *)&wimaxHdr->header;
    if (p_report_hdr->ht ==1 && p_report_hdr->ec == 1 && p_report_hdr->type == 0) {
        debug2("In sendUp(), BS receives a CQICH report from SS \n" );
        //printf("Average CINR value is %f, BLER is %f\n", avg_CINR_value,( (struct FFB_report_ext *)p->accessdata())->bler);
        addPacket(p);
        debug2("==============================BS sendUp End ===========================================\n");
        return;
    }
...
void Mac802_16BS::receive (Packet *pktRx_)
{
    ...
    if (ch->ptype() == PT_ARP) {
        debug2("Receive a ARP packet. Send to upper layer.\n");
...
//      debug2 ("CON1 :%d\n", con_tmp->get_cid());
    ...
    if ( (wimaxHdr->phy_info.OFDMSymbol_offset == 0 && wimaxHdr->phy_info.num_OFDMSymbol == 0) || wimaxHdr->header.cid == BROADCAST_CID) { // this kind of packets are treated diff(OFDM types) basically it a bw req packet.
        if (ch->error()) {
            if (collision_) {
                ...
                debug2("\t 1 error in the packet, the Mac does not process\n");
                Packet::free(pktRx_);
            }
            //update drop stat
            update_watch (&loss_watch_, 1);
        ...
        if (ch->error()) {
            ...
            //   collision_ = false;
            // } else {
            debug2("2 error in the packet, the Mac does not process");
        ...
        debug2("In BS  num of complete blocks = %d , ITU_PDP = %d, num of slots = %d, max block size = %d, last block size = %d \n" , num_of_complete_block, ITU_PDP, num_of_slots, max_block_size, last_block_size );


        if (wimaxHdr->header.cid >= 0x4001 && wimaxHdr->header.cid <=0xFEFE && amc_enable_ ==1) {
            int current_mcs_indx = 0;
            int ss_mac_id = getCManager()->get_connection(wimaxHdr->header.cid, false)->getPeerNode()->getAddr();

            current_mcs_indx = get_current_ul_mcs_index(ss_mac_id);
            debug2("In BS:current ss %d UL mcs index is %d\n",ss_mac_id, current_mcs_indx );


            if (num_of_complete_block == 0) { //only have the remain blocks, not multiple
                debug2("1.1 In BS:  Begin to calculate the BLER for the uncomplete blocks.\n");

                index =  phy->getMCSIndex( wimaxHdr->phy_info.modulation_ ,  last_block_size);
                debug2("In BS: before the amc index configure index is %d\n",index);
                ...
                set_current_ul_bler(ss_mac_id, BLER);
                ...
                debug2("1.2 In BS: Begin to calculate the BLER for the complete blocks.\n");
                ...
                set_current_ul_bler(ss_mac_id,BLER);
                //set_ss_current_mcs_index(index);
                debug2("BS set the SIR to %f,  BLER for the complete burst is [%f], mcs index is [%d]\n",SIR, BLER, index);
            ...
            /*after all the UL AMC information is done, start up UL AMC control algorithm*/
            UL_AMC_algorithm(BLER, ss_mac_id);
        } else {
            /*For the data otherwise traffic, do not bother. let it be.*/
            if (num_of_complete_block == 0) { //only have the remain blocks, not multiple
                debug2("1.4 In BS:  Begin to calculate the BLER for the uncomplete blocks.\n");
                ...
                if (num_subcarrier_block==0) {
                    debug2( "In BS: ERROR: #subcarrier_block :%d\n", num_subcarrier_block);
                    //exit(1);
                    drop(pktRx_, "WRN_SBCR");
                    return;
                }
                ...
                if (eesm_sum >= BASE_POWER) {
                    ...
                    if (SIR < 0) {
                        drop(pktRx_, "WRN_SIR");
                        debug2("In BS: WARNING: SIR less than 0.\n");
                        return;
                    }
                    SIR=10*log10(SIR);
                    debug2(" In BS:  SIR-BS in dB= %.2f = \n" , SIR );
                    BLER = global->TableLookup(index, SIR);
                } else {
                    BLER = 0;
                }
                ...
                if (!phymib_.disableInterference && BLER > rand1) {
                    ...
                    pkt_error = FALSE;
                    debug2("In BS: set channel to false.\n");
                }
            } else {
                ...
                if (eesm_sum >= BASE_POWER) {
                    ...
                    SIR=10*log10(SIR);
                    debug2(" In BS:  SIR-BS = %.2f = \n" , SIR );
                    BLER = global->TableLookup(index, SIR);
                } else {
                    BLER = 0;
                }

                debug2(" In BS: BLER at SS of the complete blocks = %.2f  \n" , BLER );
                ...
                if (last_block_size > 0) {
                    debug2("1.6 In BS: Begin to calculate the BLER for the uncomplete blocks.\n");
                    eesm_sum =0;
                    ...
                    debug2(" In BS: BLER at BS for the last block = %.2f = \n" , BLER );
...

        {
            ...
            debug2("In BS: 3 error in the packet, drop this packet. the Mac does not process\n");
            drop (pktRx_, "CNXERR");
            Packet::free(pktRx_);
            pkt_error = FALSE;
            //update drop stat
    ...
    struct mac_hdr_ffb_report *p_report_hdr = (struct mac_hdr_ffb_report *)&wimaxHdr->header;
    if (p_report_hdr->ht ==1 && p_report_hdr->ec == 1 && p_report_hdr->type == 0) {
        int avg_CINR_value = p_report_hdr->fb_content;
        debug2("BS receives a CQICH report from SS \n" );
        //printf("Average CINR value is %f, BLER is %f\n", avg_CINR_value,( (struct FFB_report_ext *)p->accessdata())->bler);
        process_ffb_report(pktRx_);
    }
...
void Mac802_16BS::UL_AMC_algorithm(float BLER, int mac_index)
{
    ...
    if (BLER <  lower_bound) { //amc_lower_bound_) /*Radio condition is good. Use more advanced modulation.*/ // Open loop Algorithm
        ...
        if (get_ul_smooth_factor(mac_index)> uL_amc_smooth_factor_) { // 2 should be configurable.
            ...
            set_ul_smooth_factor( mac_index,1);
        } else {
            printf("Anti Pingpong effect. Keep stable.\n");
            if (get_change_ul_modulation_flag( mac_index) == TRUE && get_ul_smooth_factor( mac_index) == 1) {
                debug2("UL AMC already sets the change command. Doing nothing now.\n");
            } else {
                //set_change_ul_modulation_flag(mac_index,FALSE);
        ...
        printf("UL AMC 1.3 keep unchanged modulation.SS %d, keep mod\n ", mac_index);
        set_ul_smooth_factor( mac_index,get_ul_smooth_factor(mac_index)+1);
    }
}


void Mac802_16BS::AMC_algorithm(Packet * p, struct mac_hdr_ffb_report * ffb_report)
{
    //struct hdr_cmn *ch = HDR_CMN(p);
    //struct hdr_mac802_16 *wimaxHdr = HDR_MAC802_16(p);

    //int incoming_cid = wimaxHdr->header.cid;
    //int mac_index = getCManager()->get_connection(incoming_cid,IN_CONNECTION)->getPeerNode()->getAddr();

    int avg_CINR_value = ffb_report->fb_content;

    struct FFB_report_ext *pffb_rpt = (struct FFB_report_ext *) p->accessdata();
    ...
    if (mac_index == MAX_SYNC_CQICH_ALLOC_INFO) {
        debug2("[FATAL ERROR] mac_index is not valid after lookup by cqich_id. Double check cqich_id is valid.\n");
        return;
    }
    ...
    case FFB_DL_AVERAGE_CINR:
        ...
        if (BLER < lower_bound) /*Radio condition is good. Use more advanced modulation.*/ { // Open loop Algorithm
            ...
            if (get_smooth_factor(mac_index)> dl_amc_smooth_factor_) { //configurable.
                ...
                set_increase_modulation(mac_index, TRUE);
                set_smooth_factor( mac_index,0);
            } else {
                debug2("Anti Pingpong effect. Keep stable.\n");
            ...
            if (get_smooth_factor(mac_index)>-1) { // once need to decrease, do it immediately. will overlook the smooth factor.
                ...
                set_change_modulation_flag(mac_index,TRUE);
                set_increase_modulation(mac_index, FALSE);
                set_smooth_factor( mac_index,0);
            } else {
                debug2("DL AMC: Anti Pingpong effect. Stay stable.\n");
                set_change_modulation_flag(mac_index,FALSE);
                set_smooth_factor( mac_index,get_smooth_factor(mac_index)+1);
            }
        } else { // remain unchanged.
            debug2("keep unchanged modulation.SS %d, keep mod\n ", mac_index);
            set_change_modulation_flag(mac_index,FALSE);
        }
        break;

    case FFB_MIMO_PRECODING:
    case FFB_MIMO_MODE:
    case FFB_UEP:
    case FFB_BAND_AMC_CINR:
    case FFB_INDICATION_FLAG:
        // Doing nothing for the moment.
        break;

    default:
        debug2("[Mac802_16BS: Error] Invalid Fast feedback report type.\n");
        break;
    }
}

void Mac802_16BS::process_ffb_report ( Packet * p)
{
    hdr_mac802_16 *wimaxHdr = HDR_MAC802_16(p);
    gen_mac_header_t header = wimaxHdr->header;
...
void Mac802_16BS::process_mac_packet (Connection *con, Packet * p)
{
    ...
    struct mac_hdr_cqich_alloc_request *req_tmp = (struct mac_hdr_cqich_alloc_request *)&header;

    if (header.ht ==1 && header.ec ==0) {
        if (req_tmp->type == 7) {
            debug2("BS get the CQICH request message.\n");
            debug2("amc_enable_ is %d\n", amc_enable_);
            if (amc_enable_ == 1)
                process_cqich_alloc_req(p);
            else
                debug2("AMC feature is not enabled.\n");
            return;
        }
    }
...
void Mac802_16BS::process_ranging_req (Packet *p)
{
    ...
    if (HDR_MAC802_16(p)->header.cid != INITIAL_RANGING_CID) {
        ...
        if (peer==NULL) {
            ...
            peer->setUIUC (req->req_dl_burst_profile-DIUC_PROFILE_1+UIUC_PROFILE_1);  //xingting added to support UIUC configuration separately.
            int ss_mac_id = peer->getAddr();
            set_current_mcs_index(ss_mac_id, req->req_dl_burst_profile); // must set here. otherwise in bsscheduler, the first value will be invalid.
...
/**
    ...
    */
void Mac802_16BS::process_cqich_alloc_req(Packet *p)
{
    hdr_mac802_16 *wimaxHdr = HDR_MAC802_16(p);
    gen_mac_header_t mac_header = wimaxHdr->header;
    PeerNode *peer = NULL;
    mac_hdr_cqich_alloc_request *cqich_alloc_req;
    cqich_alloc_req = (mac_hdr_cqich_alloc_request *)&mac_header;


    peer = getCManager()->get_connection (cqich_alloc_req->cid, IN_CONNECTION)->getPeerNode();
    int ss_mac_id =  peer->getAddr();
    debug2("BS received a CQICH Allocation Request from SS [%d].\n",ss_mac_id);

    /*Fill out the necessary fields in the CQICH  Allocation IE in UL-MAP IE.*/
    /*Since the filling out work willl be done at bsscheduler, here we simply set
       a flag to indicate the bsscheduler to do the filling out work. And also we need
       to specify the information for scheduler to fill.*/

    /*Update the Status of SS for AMC*/
    set_registered_flag(ss_mac_id, true);

    debug2("BS is gonna send out CQICH allocation IE.\n");
    //cqich_alloc_info_buffer[ss_mac_id].cqich_id = 1; // test only.  using num_cqich_alloc_ie_
    int alloc_offset = 0;
    int cqich_id = 0;
    cqich_id_offset_allocation(ss_mac_id, alloc_offset, cqich_id);

    cqich_alloc_info_buffer[ss_mac_id].cqich_id = cqich_id;
    cqich_alloc_info_buffer[ss_mac_id].alloc_offset = alloc_offset;
    debug2("CQICH ID is %d\t  CQICH_alloc_offset is %d\n", cqich_id, alloc_offset);


    cqich_alloc_info_buffer[ss_mac_id].ext_uiuc = UIUC_CQICH_ALLOCATION_IE; // cqich alloc ie
    cqich_alloc_info_buffer[ss_mac_id].period = bs_ffb_report_period_ ;// 0; //every frame
    cqich_alloc_info_buffer[ss_mac_id].duration = bs_ffb_report_duration_;//FFB_ENDLESS_REPORT; //endless
    cqich_alloc_info_buffer[ss_mac_id].fb_type = bs_ffb_report_type_;//FFB_DL_AVERAGE_CINR;
    cqich_alloc_info_buffer[ss_mac_id].cid =cqich_alloc_req->cid;
    debug2("period %d    duration %d     report_type %d \n", bs_ffb_report_period_,bs_ffb_report_duration_, bs_ffb_report_type_ );
    cqich_alloc_info_buffer[ss_mac_id].has_sent=false;

    /*in Bsscheduler, it will add the CQICH Allocation IE info into the corresponding
       burst and let the certain SS knows about it.*/
    set_cqich_alloc_ie_ind( true);
    return;
}

void  Mac802_16BS::cqich_id_offset_allocation(int ss_mac_id, int & alloc_offset, int & cqich_id)
{
    bool duplicated_cqich_id = FALSE;

    if (cqich_slot_allocation_offset_<  MAX_SYNC_CQICH_SLOT) {
        alloc_offset = cqich_slot_allocation_offset_;
        cqich_alloc_info_buffer[ss_mac_id].alloc_offset = 	cqich_slot_allocation_offset_++;


        u_int16_t tmp_cqich_id = 0;

        while (1) {
            /*allocated CQICH ID*/
            tmp_cqich_id  = rand() % 1024+1;   // as least it should be 1, because 0 is reserve for error allocation.

            /*make sure this cqich_id has not been used.*/
            for (int i=0; i<MAX_SYNC_CQICH_ALLOC_INFO; i++) {
                if (cqich_alloc_info_buffer[i].cqich_id == tmp_cqich_id) {
                    duplicated_cqich_id = TRUE;
                    break;
                }
            }

            if (duplicated_cqich_id != TRUE) {
                cqich_alloc_info_buffer[ss_mac_id].cqich_id = tmp_cqich_id;
                cqich_id = tmp_cqich_id;
                break;
            }
        }
    } else
...
int Mac802_16BS::lookup_ssid_via_cqich_id(int cqich_id)
{
    debug2("Lookup CQICH_id %d in database.\n", cqich_id);
    for (int i=0; i<MAX_SYNC_CQICH_ALLOC_INFO; i++) {
        debug2("cqich_id is %d\n", cqich_alloc_info_buffer[i].cqich_id);
        if (cqich_alloc_info_buffer[i].cqich_id == cqich_id) {
            return i;
        }
    }
    return MAX_SYNC_CQICH_ALLOC_INFO;
}
...
void Mac802_16BS::process_reg_req (Packet *req)
{
    ...
    //set_current_mcs_index(peer->getAddr(), 30);
...
void Mac802_16BS::start_dlsubframe ()
{
    ...
    ((BSSchedulerStd*)scheduler_)->schedule();
