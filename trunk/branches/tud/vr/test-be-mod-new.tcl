...
Mac/802_16 set enable_interfence_ 0       	;# 0 - disbale / 1 - enable interference/BLER computation
Mac/802_16 set admission_control_enable_ 0	;# 0 - disable / 1 - enable admission control
...
for {set i 0} {$i < $nb_mn} {incr i} {
    ...
    [$wl_node_($i) set mac_(0)] add-flow UL $opt(be_sf_type) $opt(be_sf_traffic_priority) \
    ...
    [$wl_node_($i) set mac_(0)] add-flow DL $opt(be_sf_type) $opt(be_sf_traffic_priority) \
					    $opt(be_sf_mstr) $opt(be_sf_max_burst_size)	$opt(be_sf_mrtr) $opt(be_sf_regtrans_policy) \
					    $opt(be_sf_tolerated_jitter) $opt(be_sf_max_latency) $opt(be_sf_sdu_indicator) $opt(be_sf_sdu_size) \
					    $opt(be_sf_arq_enable) $opt(be_sf_arq_window) $opt(be_sf_arq_timer) $opt(be_sf_arq_proctime) $opt(be_sf_arq_blocksize) \
					    $opt(be_sf_grant_interval) $opt(be_sf_polling_interval) $opt(be_sf_time_base) $opt(be_sf_error_rate)

#    $ns at 0.9 "[$wl_node_($i) set mac_(0)] add-dynamicflow UL $opt(be_sf_type) $opt(be_sf_traffic_priority) \
#					    $opt(be_sf_mstr) $opt(be_sf_max_burst_size)	$opt(be_sf_mrtr) $opt(be_sf_regtrans_policy) \
#					    $opt(be_sf_tolerated_jitter) $opt(be_sf_max_latency) $opt(be_sf_sdu_indicator) $opt(be_sf_sdu_size) \
#					    $opt(be_sf_arq_enable) $opt(be_sf_arq_window) $opt(be_sf_arq_timer) $opt(be_sf_arq_proctime) $opt(be_sf_arq_blocksize) \
#					    $opt(be_sf_grant_interval) $opt(be_sf_polling_interval) $opt(be_sf_time_base) $opt(be_sf_error_rate)"

 #   $ns at 1.0 "[$wl_node_($i) set mac_(0)] add-dynamicflow DL $opt(be_sf_type) $opt(be_sf_traffic_priority) \
#					    $opt(be_sf_mstr) $opt(be_sf_max_burst_size)	$opt(be_sf_mrtr) $opt(be_sf_regtrans_policy) \
#					    $opt(be_sf_tolerated_jitter) $opt(be_sf_max_latency) $opt(be_sf_sdu_indicator) $opt(be_sf_sdu_size) \
#					    $opt(be_sf_arq_enable) $opt(be_sf_arq_window) $opt(be_sf_arq_timer) $opt(be_sf_arq_proctime) $opt(be_sf_arq_blocksize) \
#					    $opt(be_sf_grant_interval) $opt(be_sf_polling_interval) $opt(be_sf_time_base) $opt(be_sf_error_rate)"
    ...
    set udp1_($i) [new Agent/UDP]
    $udp1_($i) set packetSize_ 256


    if { $direction == "ul" } {
	$ns attach-agent $wl_node_($i) $udp1_($i)
    } else {
	$ns attach-agent $sinkNode $udp1_($i)
    }

    # Create a CBR traffic source and attach it to udp0
    set cbr1_($i) [new Application/Traffic/CBR]
    $cbr1_($i) set packetSize_ 256
    $cbr1_($i) set interval_ $gap_size
    $cbr1_($i) attach-agent $udp1_($i)

    #create an sink into the sink node

    # Create the Null agent to sink traffic
    set null1_($i) [new Agent/Null] 
    if { $direction == "ul" } {    
	$ns attach-agent $sinkNode $null1_($i)
    } else {
	$ns attach-agent $wl_node_($i) $null1_($i)
    }
    
    # Attach the 2 agents
    $ns connect $udp1_($i) $null1_($i)
...
for {set i 0} {$i < $nb_mn} {incr i} {
    ...
    $ns at [expr $traffic_start+$i*$diff+$diff] "$cbr1_($i) start"
    $ns at [expr $traffic_stop+$i*$diff+$diff] "$cbr1_($i) stop"
