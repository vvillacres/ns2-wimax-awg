 ...
 * $Id: asim.cc,v 1.14 2014/12/17 22:16:08 tom_henderson Exp $
 *
...
public:
  ...
  int command (int , const char*const* argv){
    ...
    if (strcmp(argv[1], "readinput") == 0) {
      ...
      //std::cout << "All inputs properly obtained from " << argv[2] <<std::endl ; 
      return (TCL_OK);
    }
  ...
  for(i=0;i<nLinks;i++){
	...
	for(int k=0;k<nAdj[j];k++){
	  if(Adj[j][k]==i){
	    ...
	    //	    std::cout << links[i].theflows[c-1] << sp;
	  }
	}
      }
      // std::cout << " slambda = " << links[i].lambda;
    }
...
//std::cout << "rho = " << rho << " drop = " << links[i].drop << std::endl;
	...
	*/
    ...
    //std::cout << "delay = " << links[i].qdelay << " and drop = " << links[i].drop << std::endl;
...
void CalcPerFlowDelays(){
  for(int i=0; i<nConnections; i++){
    ...
    if(flows[i].is_sflow){
      ...
      if(!p){
	// std::cout << "Oops, something wrong";
      }
      flows[i].p_tput = padhye(d,p);
    }

    //    std::cout << "connection " << sp << i << sp << d << sp << p; 
    //std::cout << sp << flows[i].p_tput << std::endl;
...
void PrintData(){
  for(int i=0;i<nLinks;i++){
    std::cout << i << sp << links[i].lambda << sp << links[i].mu;
    std::cout << sp << links[i].buffer << std::endl;
  }
}
...
void PrintResults(){
  ...
  for(i=0;i<nLinks;i++){
    // std::cout << i << sp << links[i].qdelay << sp << links[i].drop;
    std::cout << sp << "Qdelay = " << links[i].prop << sp << links[i].lambda;
    std::cout << sp << links[i].drop << std::endl;
  }

  for(i=0; i<nConnections; i++){
    std::cout << i << sp << flows[i].delay << sp;
    std::cout << flows[i].drop << sp << flows[i].p_tput << sp;
...
int allscaled(){
  ...
  //std::cout << nConnections;

  for(int i=0; i<nConnections; i++)
    if(!flows[i].is_sflow && !flows[i].scaled){
      std::cout << "Connection " << i << " not scaled as yet\n";
      return 0;
    }

  //std::cout << "All are scaled\n";
...
// flag = 1 means dont touch short flows in your scaling algo
  ...
  for(i=0; i<nLinks; i++){
    //std::cout << "after updatehelper link #" << i << " " << links[i].tlambda << "\n";
  ...
  std::cout << "bottleneck = " << bneck << sp << maxtlambda <<std::endl;
  ...
  while((maxtlambda > tk + 1) && ! allscaled()){
    ...
    std::cout << "Maxtlambda = " << maxtlambda << " bneck = " << bneck << std::endl;

    //    std::cout << "tk =  "<< tk << " maxlambda = " << maxtlambda << std::endl;
	...
	if(!flows[t].is_sflow && !flows[t].scaled){
	  ...
	  //std::cout << "Flow " << t << " getting scaled to  << " << flows[t].p_tput <<" \n";
    ...
    for (i=0; i<nLinks;i++){
      std::cout << "Link " << i << " tlambda = " << links[i].tlambda << std::endl;
    }
  ...
  Update(0);
  std::cout << "Out of the converge loop\n";
    for (i =0; i<nLinks;i++){
      std::cout << "Link " << i << " tlambda = " << links[i].tlambda << std::endl;
    }
...
void newupdate(int niter){
  ...
  //  for(i=0; i<nLinks; i++ ){
  //std::cout << i << sp << links[i].uc << sp << links[i].utput << std::endl;
  //}
  ...
  */
  ...
  while(bneck+1){
    ...
    // std::cout << "bneck = " << bneck << sp << links[bneck].uc << sp << links[bneck].utput << sp << maxgamma << sp << links[bneck].nflows <<std::endl;
    ...
    // std::cout << links[bneck].uc << sp << links[bneck].utput << std::endl;
...
int main(int argc, char **argv) {
  ...
  //  PrintResults();
  std::cout << "Read the input .... \n";

  for(int i=0; i<10; i++){
    sim.CalcLinkDelays(1);
    std::cout << "Calculated link delays ... \n";
    //    PrintResults();

    sim.CalcPerFlowDelays();
    std::cout << "Calculated per flow delays ... \n";
    std::cout << " ------------------------------\n";
    sim.newupdate(niter);
    //sim.PrintResults();
    std::cout << " ------------------------------\n"; 
