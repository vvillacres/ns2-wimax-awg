...
TrafficShapingMsadaa::TrafficShapingMsadaa( double frameDuration )  : TrafficShapingInterface( frameDuration)
{
    // Nothing to do

}

TrafficShapingMsadaa::~TrafficShapingMsadaa()
{
    // Nothing to do
}
 ...
 */
MrtrMstrPair_t TrafficShapingMsadaa::getDataSizes(Connection *connection, u_int32_t queuePayloadSize)
{
    ...
    double maxLatency = sfQosSet->getMaxLatency() * 1e-3;

    int cirBucketSize = int( ceil(double(sfQosSet->getMinReservedTrafficRate()) / 8.0 * maxLatency));
    int eirBucketSize = int( floor(double(sfQosSet->getMaxSustainedTrafficRate() - sfQosSet->getMinReservedTrafficRate()) / 8.0 *maxLatency));
    ...
    if ( mapIterator == mapLastBucketSize_.end()) {
    	...
    	cirToken = int( ceil( double(sfQosSet->getMinReservedTrafficRate()) * maxLatency / 8.0 ) );
    	cirToken = MINTS(cirToken, cirBucketSize);

        // new eir tokens for the first time
    	eirToken = int( floor( double(sfQosSet->getMaxSustainedTrafficRate() - sfQosSet->getMinReservedTrafficRate()) * maxLatency/ 8.0 ) );
 ...
 */
void TrafficShapingMsadaa::updateAllocation(Connection *con,u_int32_t realMrtrSize,u_int32_t realMstrSize)
{
