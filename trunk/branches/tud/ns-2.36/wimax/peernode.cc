 ...
 */
PeerNode::PeerNode (int index):
		current_mcs_index_ (0),
		current_ul_mcs_index_ (0),
		peerIndex_ ( index),
 	 	basic_in_ (0),
		basic_out_ (0),
        primary_in_ (0),
 ...
 * @param diuc The downlink profile
 */
void  PeerNode::setUIUC (int uiuc)
{
    printf("BS set the UIUC for connection to %d\n", uiuc);
    uiuc_=uiuc;
}
